...
const Connections = () => {
  ...
  return (
    <div className='h-full overflow-y-scroll no-scrollbar min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className="mb-8 flex flex-wrap items-center justify-center gap-6">
          {dataArray.map((item,index)=>(
            <div key={index} className="flex flex-col items-center justify-center gap-1 border h-20 w-40 border-gray-200 bg-wgite shadow rounded-md">
        ...
        {/* Tabs */}
        <div className="flex flex-wrap items-center justify-evenly border border-gray-200 rounded-md bg-white shadow-sm">
          {
            dataArray.map((tab)=>(
            ...
            <div key={user._id} className='w-full max-w-88 flex flex-col text-center gap-5 p-6 bg-white shadow rounded-md'>
              ...
              <div className="flex-1">
                ...
                <div className="flex max-sm:flex-col gap-2 mt-4">
                  {
                    <button onClick={() => navigate(`/profile/${user._id}`)} className='w-full p-2 text-sm rounded bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:sclae-95 transition text-white cursor-pointer'>
                      View Profile
                    </button>
                  }
                  {
